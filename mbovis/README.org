...
# ENA Study Accession: PRJEB38523
# ENA Run Accession: ERR4179766
# only reads >15bp
seqkit seq -m 15000 M_bovis_MinION_Bonito_Subsampled/PG45.fastq |
  seqkit fq2fa > mbovis-reads-raw.fa

# For visualization we need the location of a reads on the reference genome. Use
# only reads fully aligned (90% mapped, primary), and forward - makes it easier
# to extract the right genes from the reference genome
